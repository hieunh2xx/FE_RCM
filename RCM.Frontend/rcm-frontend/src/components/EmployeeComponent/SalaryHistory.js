...
const SalaryHistory = () => {
  ...
  const fetchPayrollData = async () => {
    try {
      let url = `https://localhost:5000/api/Payroll/getAllPayroll?month=${selectedMonth}&year=${selectedYear}`;
  ...
  const exportFile = async () => {
    try {
      const response = await fetch(
        `https://localhost:5000/api/Payroll/export?month=${selectedMonth}&year=${selectedYear}`,
        {
          method: "GET",
          headers: {
  ...
  const handlePayment = async () => {
    try {
      const response = await fetch(
        "https://localhost:5000/api/Payroll/pay-salary",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const openDetailModal = async (employeeId) => {
    try {
      const response = await fetch(
        `https://localhost:5000/api/Payroll/details?employeeId=${employeeId}&month=${selectedMonth}&year=${selectedYear}`
      );
  ...
  const updateSalary = async () => {
    try {
      const response = await fetch(
        "https://localhost:5000/api/Payroll/update-salary",
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
